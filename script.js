...
function fetchCountryCodes() {
  fetch("https://flagcdn.com/en/codes.json")
    ...
    .catch((error) => console.error("Error fectching country codes:", error));
}
...
function fetchFlag(countryCode) {
  fetch(`https://flagcdn.com/256x192/${countryCode}.png`)
    ...
    .catch((error) => {
      console.error("Error fectching flag", error);
      document.getElementById("flag-result").classList.remove("hidden");
      document.getElementById("flag-image").alt =
        "Error fectching flag. Please try again later.";
    });
}
